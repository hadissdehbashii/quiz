...
const Modal = ({ onClose, questionId, questionText }: modalProp) => {
  ...
  return (
    <div onClick={onClose}>
      ...
      <div  className="fixed inset-0 z-40" style={{ opacity: 0.5, background: 'rgba(0,0,0,0.5)' }}></div>
      <div className="fixed inset-0 flex items-center justify-center z-50">
        <div onClick={handleContentClick} className=" m-10 rounded-xl py-8 px-10 md:m-80 bg-base-100 md:w-[500px]">
          ...
          <div className="flex justify-end mt-4 gap-3">
            ...
            <button disabled={isPending} className="btn btn-outline btn-error"
              onClick={() => deleteQuestion(questionId, { onSuccess: onClose })}>{isPending ? 
              <span className="loading loading-spinner text-error"></span> : "Submit"}</button>
          </div>
        </div>
      </div>
